...
void rb_print(const struct RingBuffer *const rb) {
  // printf("b:%p c:%d, l:%d, i:%d, r:%d  ", rb->buffer, rb->capacity,
  // rb->length,
  //        rb->insertIdx, rb->removeIdx);

  printf("[");
  int idx, elem;
  for (elem = 0, idx = rb->removeIdx; elem < rb->length;
       elem++, idx = (idx + 1) % rb->capacity) {
...
void rb_push(struct RingBuffer *rb, int val) {
  assert(rb->length < rb->capacity &&
         "Bounds error: Attempted to push an element into a full buffer");
...
int rb_pop(struct RingBuffer *rb) {
  assert(rb->length > 0 &&
         "Bounds error: Attempted to pop an element from an empty buffer");
